...
- `canaryctl`: direct-to-SQLite CLI for writing and querying review marks, rationale notes, threads, and file briefs
- `skills`: a canonical installable instruction bundle that teaches the model when and how to call `canaryctl`

The goal is to help the engineer understand generated code, keep a mental model of the evolving codebase, and see exactly which lines merit review because the agent made a design decision, assumption, or tradeoff.
...
- `packages/canaryctl/`: annotation and search CLI
...
Project-local state is written under `.canary/`, with the active SQLite database at `.canary/canary.db`.
...
- `node packages/canaryctl/dist/cli/index.js search "<query>"`
...
The demo workspace is recreated under `live-demo/workspace/` on each run, and its SQLite database lives at `live-demo/workspace/.canary/canary.db`.
