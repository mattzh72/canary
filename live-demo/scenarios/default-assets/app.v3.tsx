...
export async function App(): Promise<string> {
  ...
  return [
    ...
    `    <p>${summary.overloadedReviewers} reviewers above the queue threshold</p>`,
    "  </header>",
    "  <main>",
    ...
    staleDraft
      ? `    <p>Escalate ${staleDraft.title}; the queue is already stacked against that reviewer.</p>`
      : summary.overloadedReviewers > 0
        ? "    <p>Review load is clustering even without a stale handoff yet.</p>"
